...
O sistema tem três interfaces distintas:

1. **Loja** (`chat-dolar-store/`) — vitrine de produtos estática
2. **Chat** (`chat-dolar-frontend/`) — app React com painel admin e chat de visitante
3. **Backend** (`chat-dolar-backend/`) — API Laravel + WebSockets via Reverb

---

## Sellers (Tenants)

Cada seller tem um `tenant_id` UUID fixo. A loja é partilhada mas os chats são separados por tenant.
...
| Admin painel      | `http://192.168.1.100:5173/leads`                      |
...
- `productId` = `public_id` do produto (ex: `AGsjGAjskalsksl`)
...
- Username guardado no `localStorage` por tenant

### 4. Mensagens automáticas injectadas pelo backend
Ao criar a conversa, o `StartConversationAction` injeta automaticamente 3 mensagens:

**Mensagem 1 — do visitante (auto):**
```
Hi! I want to buy: FIFA 26 PC ($35)
```

**Mensagem 2 — do admin/bot (PaymentBubble):**
Renderizada como componente visual especial (`PaymentBubble.tsx`) no frontend.
Detectada pelo prefixo `"The safest and most private payments"`.
Contém: BTC address (Binance), Gift Cards (Eneba + G2A), PayPal address.
Os endereços são extraídos do campo `btc_address` e `paypal_address` do tenant na DB.
...
### Gestão de produtos
Os produtos estão definidos em **dois lugares** que devem estar sempre sincronizados manualmente:

| Ficheiro | Função |
|----------|--------|
| `chat-dolar-store/store.js` | Lista hardcoded para a loja (HTML puro, sem API) |
| `database/seeders/ProductSeeder.php` | Seed para a DB (usado pelo backend para gerar mensagens automáticas) |

> **Nota:** Não existe API de produtos para a loja. Ao adicionar um produto novo, é necessário editar ambos os ficheiros.

### Estrutura na DB (tabela `products`)
...
## Endereços de Pagamento por Tenant

Guardados na tabela `tenants`. Usados pelo `StartConversationAction` para gerar a mensagem de pagamento.

| Tenant  | btc_address                          | paypal_address       |
|---------|--------------------------------------|----------------------|
| Kodan   | *(vazio — a preencher)*              | *(vazio)*            |
| Phoenix | `1MPa1fSFYYWADLC7xvZpXh5VM1mDp2mjGD`| `seller@paypal.com`  |
...
app/Models/Tenant.php                          — model com btc_address e paypal_address
app/Http/Controllers/Api/ProductController.php — GET /products/{publicId}?tenant_id=
database/seeders/DatabaseSeeder.php            — tenants + users com UUIDs fixos
database/seeders/ProductSeeder.php             — produtos do tenant Phoenix
database/migrations/0001_01_01_000001_create_tenants_table.php
database/migrations/0001_01_01_000003_create_app_tables.php
```
...
src/components/chat/PaymentBubble.tsx    — bubble especial de pagamento (BTC, Gift Cards, PayPal)
src/components/chat/MessageBubble.tsx    — bubble genérico (detecta e delega para PaymentBubble)
...
├── store.js      — lógica, produtos hardcoded, filtros, carousel
